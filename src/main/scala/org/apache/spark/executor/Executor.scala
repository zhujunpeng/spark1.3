  ...
  extends Logging
{
  ...
  def launchTask(
    ...
    // 使用线程池启动这个taskRunnable 自动实现排队机制
  ...
  class TaskRunner(
    ...
    override def run() {
      ...
      try {
        ...
        // 执行task
        // 如果是shuffleMaptask那么返回值就是Mapstatus，这里面封装了shuffleMapTask计算的数据，输出位置
        // 如果后面还是shuffleMapTask,那么就会联系MapOutPutStacker，
        // 获取上一个shuffleMapTask计算输出的数据源，位置，然后通过网络获取数据 是ResultMapTask也一样
        ...
        // 计算出task的一些统计信息，包括运行多久，反序列化耗费时间，JVMGC时间，结果序列化操作
        // 这些信息会在sparkUI上显示信息
        for (m <- task.metrics) {
        ...
        // 调用了executor的statusUpdate方法，更新信息
