  ...
  extends ExecutorAllocationClient with SchedulerBackend with Logging
{
  ...
  class DriverActor(sparkProperties: Seq[(String, String)]) extends Actor with ActorLogReceive {
    ...
    def makeOffers() {
      // 第一步、调用TaskSchedulerImpl的resourceOffers方法，执行任务分配算法，将task分配到executor上去
      // 第二步、分配好task到executor上去之后，执行launchTasks方法，将分配的task发送launch消息到对应的executor上去，由executor启动并执行task
      // 参数为这个application所有可用的executor，并将其封装成WorkerOffer，每个WorkerOffer代表了每个executor可用的cpu资源数
    ...
    /**
     * 根据分配好的task，在executor上启动相应的task
     */
    def launchTasks(tasks: Seq[Seq[TaskDescription]]) {
      for (task <- tasks.flatten) {
        // 将每个executor要执行的task信息，统一进行序列化操作
        ...
        if (serializedTask.limit >= akkaFrameSize - AkkaUtils.reservedSizeBytes) {
          ...
          // 从executor上减去将要使用的cpu资源
          executorData.freeCores -= scheduler.CPUS_PER_TASK
          // 发送消息，在executor上启动task
