...
class DAGScheduler(
   ...
   * 
   * 计算每个task的partition的最佳位置
   * 从stage的最后一个RDD开始，去那个rdd的partition是被cache了还是checkpoint了，那么最佳位置就是cache/checkpoint那个位置
   * 因为这样的话task就在这个节点上执行就不要再次计算以前的RDD了。
   * 优先查找cache，再找checkpoint的partition
   * 循环遍历
   */
    ...
    : Seq[TaskLocation] =
  {
    ...
    // 如果返回了Nil，那么就是没有最佳位置，那么最后将由taskscheduler决定
    Nil
  }
